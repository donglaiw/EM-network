    ...
    for _, volume, label, class_weight, _ in train_loader:
        ...
        if volume_id % args.volume_valid < args.batch_size or volume_id >= args.volume_total:
            ...
            with torch.no_grad():
                ...
                # volume.shape = (batch_size, 1, z, x, y)
                # label.shape = (batch_size, num_classes, z, x, y)
                # output.shape = (batch_size, num_classes, z, x, y)
                ...
                writer.add_image('Validation Label', vutils.make_grid(label.detach()[:,2,volume.shape[2]//2:volume.shape[2]//2+1]), volume_id)
                writer.add_image('Validation Output', vutils.make_grid(output.detach()[:,2,volume.shape[2]//2:volume.shape[2]//2+1]), volume_id)
