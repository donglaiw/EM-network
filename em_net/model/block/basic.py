...
def getBN(out_planes, dim=1, mode='single'):
    if mode == 'async':
        if dim == 1:
        ...
        if dim == 1:
            return nn.SynchronizedBatchNorm1d(out_planes)
        elif dim == 2:
            return nn.SynchronizedBatchNorm2d(out_planes)
        elif dim == 3:
            return nn.SynchronizedBatchNorm3d(out_planes)
    raise ValueError('Unknown BatchNorm option: '+str(bn_mode))

def conv3dBlock(in_planes, out_planes, kernel_size, stride, padding, bias=[True], pad_mode=['zero'], bn_mode=['async'], relu_mode=['relu'], init_mode='kaiming_normal', dilation_size=None):
    ...
    for i in range(len(in_planes)):
        ...
        if len(bn_mode[i])>0:
            layers.append(getBN(out_planes[i], 3, bn_mode[i]))
...
def upsampleBlock(in_planes, out_planes, up=(1,2,2), mode='bilinear',
        ...
        if mode == 'bilinear':
            out = nn.Sequential(
                nn.Upsample(scale_factor=up, mode='trilinear', align_corners=True),
            ...
            out = nn.Sequential(nn.ConvTranspose3d(
